...
for l=1:length(plot_periods)
    chosen_period = plot_periods(l);
...
for i=1:size(setup_combs, 1) % Iterate over different combinations of statistic
    ...
    for l=1:length(plot_periods)
        ...
        elseif any(ismember(setup_comb, 15:20))
        ...
        for j=1:2  
            ...
            if isempty(ax_objs{j})
                ...
                [~, c_obj] = contour(ax_objs{j}, reshape(xi(:,1), ks_num_points, ks_num_points), reshape(xi(:,2), ks_num_points, ks_num_points), ...
                    reshape(f, ks_num_points, ks_num_points), [0.15, 0.05], 'b', 'LineWidth', 2, 'ShowText', 'off'); drawnow;
                set(c_obj, 'LineColor', uint8(c_map(l, :)'.*255));
            else
                hold(ax_objs{j}, 'on'); 
                [~, c_obj] = contour(ax_objs{j}, reshape(xi(:,1), ks_num_points, ks_num_points), reshape(xi(:,2), ks_num_points, ks_num_points), ...
                    reshape(f, ks_num_points, ks_num_points), [0.15, 0.05], 'b', 'LineWidth', 2, 'ShowText', 'on'); drawnow; 
                set(c_obj, 'LineColor', uint8(c_map(l, :)'.*255));
                hold(ax_objs{j}, 'off'); 
            end
